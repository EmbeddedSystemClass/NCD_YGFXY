...
unsigned char USB_STATUS_REG=0;
////////////////////////////////////////////////////////////////////////////////
...
{     
	if(lun==0)
	{
		...
		*block_num=SD_CardInfo.Capacity / SD_BLOCK_SIZE;
	}else
	{
...
int8_t STORAGE_Read (uint8_t lun,uint8_t *buf,uint32_t blk_addr,uint16_t blk_len)
{
	...
	if(lun==0)
	{
 		res = SD_ReadMultiBlocks(buf, blk_addr, SD_BLOCK_SIZE, blk_len);
...
int8_t STORAGE_Write (uint8_t lun,uint8_t *buf,uint32_t blk_addr,uint16_t blk_len) 
{
	...
	if(lun==0)
	{
		res=SD_WriteMultiBlocks(buf, blk_addr, SD_BLOCK_SIZE, blk_len);
