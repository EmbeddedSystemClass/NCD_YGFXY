...
#define	UserNormalProgram				0x60					//Í¨ÓÃ³ÌĞò
#define	UserXGProgram					0x61					//Ğ¢¸ĞÖĞĞÄÒ½Ôº³ÌĞò
#define	UserProgramType					UserNormalProgram			//³ÌĞòÓÃ»§Ñ¡Ôñ¶¨Òå
...
{
	unsigned char qu1Ise;									//ÇúÏß1ÊÇ·ñÊÇÖ¸Êı
	unsigned char qu2Ise;									//ÇúÏß2ÊÇ·ñÊÇÖ¸Êı
	unsigned char qu3Ise;									//ÇúÏß3ÊÇ·ñÊÇÖ¸Êı
	float qu1_d;	
	float qu2_d;
	float qu3_d;
	unsigned char parm[5];								//Ô¤Áô
...
typedef struct DeviceInfo_Tag
{
	...
	MyBool isnew;															//Éè±¸ĞÅÏ¢ÊÇ·ñÓĞ¸üĞÂ
	unsigned short crc;
}DeviceInfo;
#pragma pack()
...
typedef struct TestLine_tag {
	...
	float t_cValue;
	float BasicResult;
	unsigned char CMdifyNum;					//cÏß²¹³¥±¶Êı£¬*10
